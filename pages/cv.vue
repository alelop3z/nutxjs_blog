  ...
  <main>
    <section class="cta-section theme-bg-light py-5">
      <div class="container text-center single-col-max-width">
        <h2 class="heading mb-3">{{ cv.title }}</h2>
        <nuxt-link :to="cv.pdf_url" class="btn btn-primary" target="_blank">
          <i class="fas fa-file-pdf mr-2"></i> Descargar versión PDF
        </nuxt-link>
      </div><!--//container-->
    </section>
...
export default {
  async asyncData ({ $content }) {
    const cv = await $content('pages/cv').fetch();
